...
jQuery.fn.animateTitle = function ( options ) {
	...
	return {
		start: function ( completeFunc ) {
			...
			TweenMax.to( $mask, 0.8, { 'width': $img.width(), ease:Expo.easeOut, delay: 0.5, onComplete: function () {
				if ( completeFunc ) {
					completeFunc();
				}
			} } );
			...
			TweenMax.to( $mask, 0.8, { 'left': '0', 'width': '0', ease:Expo.easeOut, delay: 0.5, onComplete: function () {
				if ( completeFunc ) {
					completeFunc();
				}
			} } );
